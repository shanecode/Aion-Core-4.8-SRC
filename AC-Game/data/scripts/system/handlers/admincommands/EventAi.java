        ...
        if(params[0].equals("rewardall_inzone")){
            ...
            RequestResponseHandler requestHim = new RequestResponseHandler(admin) {
                            ...
                            if (MathUtil.isInRange(admin, player, range)){
                                if(admin != player){
                                    ...
                                    if(toll != 0){
                                        ...
                                        PacketSendUtility.sendMessage(player, "You received " + toll + " toll(s) from event!");
                                    }
                                    PacketSendUtility.sendWhiteMessage(admin, "Player : " + player.getName() + " has been rewarded!!");
                                }
                            }
                        }
                    });
                ...
                public void denyRequest(Creature requester, Player responder) {
                    PacketSendUtility.sendMessage(admin, "Yeah, you better recheck wat you wrote.. lol");
                }
            };
            boolean areyousure = admin.getResponseRequester().putRequest(1300564, requestHim);
            if (areyousure){
                PacketSendUtility.sendPacket(admin, new SM_QUESTION_WINDOW(1300564, 0, 0, "##########=Reward List=##########\n" +
                        "Tempering Solution : " + TS + "\n------------------------------------" +
                        "Omega Stone : " + Omega + "\n--------------------------------------------------" +
                        "Glory Point : " + GP + "\n-----------------------------------------------------" +
                        "Tolls : " + toll + "\n----------------------------------------------------" +
                        "Everyone in Range of " + range + "\n!!!!" +
                        "SO ARE YOU SURE THAT THESE ARE THE CORRECT VALUES!?!"));
            }
